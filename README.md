...
3. 授予 App「短信」权限（接收 + 读取）。国产 ROM（小米/华为/OPPO/vivo 等）建议在系统设置中为 App 开启「自启动」并关闭电池优化，避免后台监听被系统杀死。
4. App 采用「前台服务监听短信数据库 + 系统广播」双通道捕获验证码；如自动上传失败，可在 App 内点「立即扫描最近短信」手动同步最近 15 条收件箱短信。
