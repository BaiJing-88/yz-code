...
- 服务端端口：`3000`
- 线上访问地址：`http://frp-act.com:17435`（frp 内网穿透 → 本机 3000，无 HTTPS）
- 仅支持 Android（不支持 iOS）
...
1. 启动服务端并保持 frp 在线。
2. 手机安装 App（见下），打开后服务器地址已预填 `http://frp-act.com:17435`，注册/登录账号。
3. 授予 App「短信」权限（接收 + 读取）。
4. 手机收到验证码短信后自动上传；浏览器打开 `http://frp-act.com:17435` 登录同一账号即可实时查看（每 4 秒自动刷新）。
...
- 服务端为 HTTP 明文传输，请勿在不可信网络环境暴露；建议仅在个人 frp 通道使用。
